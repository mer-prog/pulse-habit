...
多くの習慣アプリは現実世界で壊れる — 地下鉄、機内モード、不安定なWi-Fi。PulseHabitはこの問題を解決するために作った。全ての変更はまずSQLiteに書き込み、Supabaseへは非同期で同期するため、UIは通信状態に関係なくネットワークを待たずに応答する。
...
| スキル | 実装内容 |
|--------|---------|
| **オフラインファースト設計** | 全ての変更をまずローカルSQLiteに書き込み、キュー経由でSupabaseに非同期同期。指数バックオフ（失敗時刻を記録し、待機時間経過までリトライをスキップ）＋自動パージで接続状態に関係なくデータ消失ゼロ。 |
...
計算は習慣の頻度設定を尊重する: daily は毎日の連続、weekly / custom は `target_days`（曜日指定）の**スケジュール日のみ**で連続性を判定。非スケジュール日はストリークを壊さず、オフ日の完了はストリークを増やしも壊しもしない。「今日が未完了」はその日が終わるまでストリークを壊さない（猶予）。
...
本コードベースに対して静的コードレビューによる**セルフ監査**を実施（第三者監査ではない）。結果:
...
| ID生成 | **NOTE** | 主経路は `crypto.getRandomValues()`。Web Crypto非対応環境向けに `Math.random()` フォールバックが残存（既知の制限） |
...
| 予測可能なID | 主経路で`crypto.getRandomValues()`を使用（フォールバックは上記NOTE参照） |
